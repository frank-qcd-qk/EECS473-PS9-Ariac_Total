...
void orderCallback(const osrf_gear::Order::ConstPtr& msg) {
    g_order = *msg;
    g_got_order = true;
    ROS_INFO("Received order %s with %i shipment%s", msg->order_id.c_str(),
             (int)msg->shipments.size(), msg->shipments.size() == 1 ? "" : "s");
    ROS_INFO_STREAM(g_order);
}

void start_competition(ros::NodeHandle& startCompetitionNode) {
    ros::ServiceClient startup_client =
        startCompetitionNode.serviceClient<std_srvs::Trigger>(
    ...
    while (!startup_srv.response.success) {
        ROS_WARN(
            "[Startup Phase]Not successful startup yet... Repeat Calling...");
        startup_client.call(startup_srv);
        ros::Duration(0.1).sleep();
    }
    ROS_INFO(
        "[Startup Phase]Competition Startup Success>>>[Belt Control Phase]");
}
...
int main(int argc, char** argv) {
    ...
    ros::ServiceClient drone_client =
        nh.serviceClient<osrf_gear::DroneControl>("/ariac/drone");
    ...
    ROS_INFO("Competition Start Complete!");
    ...
    ROS_WARN("Arrived Quality Camera 1 Area...");
    ...
    boxInspector.compute_shipment_poses_wrt_world(
        g_order.shipments[0], box_pose_wrt_world, desired_models_wrt_world);
    ...
    // Checksum
    bool checklist[4] = {false, false, false,
                         false};  // Orphane, bad, misplaced, missing
    while (accumulate(checklist, checklist + 4, 0) != 4) {
        ...
        if (boxInspector.get_bad_part_Q(current_part, CAM1)) {
            ...
            // robotBehaviorInterface.pick_part_from_box();

            // use the robot action server to acquire and dispose of the
            // specified part in the box:
        }
        ...
        ROS_INFO(
            "done removing orphans; attempt part relocations, as necessary");
        ...
        boxInspector.update_inspection(
            ...
            part_indices_misplaced, part_indices_precisely_placed, CAM1);
        nparts = orphan_models_wrt_world.size();

        int nparts_misplaced = misplaced_models_actual_coords_wrt_world.size();
        ROS_INFO("found %d misplaced parts", nparts_misplaced);
        bool go_on = false;
        for (int counter = 0; counter < nparts_misplaced; counter++) {
            model_to_part(misplaced_models_actual_coords_wrt_world[counter],
                          current_part, box_location_code);
            // adjust_part_location_no_release(Part sourcePart, Part
            // destinationPart, double timeout = MAX_ACTION_SERVER_WAIT_TIME);
            int index_des_part = part_indices_misplaced[counter];
            model_to_part(desired_models_wrt_world[index_des_part],
                          desired_part, box_location_code);
            ...
            status = robotBehaviorInterface.adjust_part_location_no_release(
                current_part, desired_part);
            status = robotBehaviorInterface.release_and_retract();
        }

        // populate missing parts  in box:
        // should repeat for ALL  missing parts
        for (int counter = 0; counter < part_indices_missing.size();
             counter++) {
            int n_missing_part = part_indices_missing[counter];

            // model_to_part(desired_models_wrt_world[n_missing_part],
            // current_part, inventory_msgs::Part::QUALITY_SENSOR_2);
            std::string part_name(
                desired_models_wrt_world[n_missing_part].type);
            ...
            part_in_inventory = binInventory.find_part(
                current_inventory, part_name, pick_part, partnum_in_inventory);
            if (!part_in_inventory) {
                ROS_WARN(
                    "could not find desired  part in inventory; giving up on "
                    "process_part()");
                return false;  // nothing more can be done
            }
            ROS_INFO_STREAM("found part: " << pick_part << endl);
            // specify place part:
            model_to_part(desired_models_wrt_world[n_missing_part], place_part,
            ...
            if (!go_on) {
                ROS_WARN(
                    "could not compute key pickup and place poses for this "
                    "part source and destination");
            }
            ...
            if (!robotBehaviorInterface.move_part_to_approach_pose(
                    place_part)) {
            ...
            cout << "enter 1: ";
            cin >> ans;
            if (!robotBehaviorInterface.place_part_in_box_no_release(
                    place_part)) {
        ...
        //bool checklist[4] = {false, false, false,
        //                 false};  // Orphane, bad, misplaced, missing
        if(part_indices_missing.size() == 0){
            checklist[3]=true;
        }
    ...
    //! Stage 1 --> Stage 2
    ROS_WARN("Going to Camera 2 Area...");
    cout << "enter 1: ";
    cin >> ans;
    // advance the box further!
    conveyorInterface
        .move_box_Q1_to_Q2();  // member function of conveyor interface to move
                               // a box to inspection station 1
    while (conveyorInterface.get_box_status() !=
    ...
    // now at location Q2; specify this for all part destinations in subsequent
    // moves:
    ...
    // void BoxInspector::compute_shipment_poses_wrt_world(osrf_gear::Shipment
    // shipment_wrt_box,
    ...
    // Checksum
    fill(begin(checklist),end(checklist),false);   checklist[4] = {false, false, false,
                         false}; // Orphane, bad, misplaced, missing
    while (accumulate(checklist, checklist + 4, 0) != 4) {
        nparts = orphan_models_wrt_world.size();
        ROS_INFO("num orphaned parts seen in box = %d", nparts);
        ...
        boxInspector.update_inspection(
            ...
            misplaced_models_desired_coords_wrt_world, missing_models_wrt_world,
            orphan_models_wrt_world, part_indices_missing,
            part_indices_misplaced, part_indices_precisely_placed, CAM2);
        nparts = orphan_models_wrt_world.size();
        ...
        for (int counter = 0; counter < part_indices_missing.size();
            ...
            // specify place part:
            model_to_part(desired_models_wrt_world[n_missing_part], place_part,
                          inventory_msgs::Part::QUALITY_SENSOR_2);
    ...
    conveyorInterface
        .move_box_Q2_to_drone_depot();  // member function of conveyor interface
                                        // to move a box to shipping dock

    while (conveyorInterface.get_box_status() !=
           conveyor_as::conveyorResult::BOX_SENSED_AT_DRONE_DEPOT) {
        ros::spinOnce();
        ros::Duration(0.1).sleep();
        nprint++;
        if (nprint % 10 == 0) {
            ROS_INFO(
                "waiting for conveyor to advance a box to loading dock...");
        }
    }
    ROS_INFO("calling drone");

    droneControl.request.shipment_type = g_order.shipments[0].shipment_type;
    ROS_INFO_STREAM("shipment name: " << g_order.shipments[0].shipment_type
                                      << endl);
